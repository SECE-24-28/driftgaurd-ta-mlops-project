...
export function TrustedBy() {
  ...
  return (
    <section className="py-12 border-y-4 border-foreground bg-primary overflow-hidden flex flex-col items-center relative">
      ...
      <div className="font-mono font-black uppercase tracking-widest text-background mb-8 text-center px-4 relative z-10 text-xl border-b-4 border-background pb-2">
      ...
      <div className="w-full inline-flex flex-nowrap overflow-hidden [mask-image:_linear-gradient(to_right,transparent_0,_black_128px,_black_calc(100%-128px),transparent_100%)] relative z-10">
        <ul className="flex items-center justify-center md:justify-start [&_li]:mx-4 [&_img]:max-w-none animate-marquee">
          {industries.map((ind, i) => (
            <li key={i} className="flex items-center gap-3 font-black text-2xl uppercase bg-background text-foreground border-4 border-foreground px-6 py-3 brutal-shadow whitespace-nowrap hover:bg-accent transition-colors">
              <ind.icon className="w-8 h-8 text-primary group-hover:text-foreground" strokeWidth={3} />
              {ind.name}
            </li>
          ))}
        </ul>
        <ul className="flex items-center justify-center md:justify-start [&_li]:mx-4 [&_img]:max-w-none animate-marquee" aria-hidden="true">
          {industries.map((ind, i) => (
             <li key={i} className="flex items-center gap-3 font-black text-2xl uppercase bg-background text-foreground border-4 border-foreground px-6 py-3 brutal-shadow whitespace-nowrap hover:bg-accent transition-colors">
              <ind.icon className="w-8 h-8 text-primary" strokeWidth={3} />
              {ind.name}
            </li>
          ))}
        </ul>
